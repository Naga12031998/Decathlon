...
class Health extends React.Component {
        ...
        window.onscroll = () => {
            ...
            if (offset === height) {
                this.timer = setTimeout(() => {
                    Axios.get(`https://newsapi.org/v2/top-headlines?country=in&category=health&apiKey=c8ca65b96c864b2aa46262c0e029520e`)
                        .then((res) => {
                            this.setState({
    ...
    componentDidMount = () => {
        Axios.get(`https://newsapi.org/v2/top-headlines?country=in&category=health&apiKey=c8ca65b96c864b2aa46262c0e029520e`)
            .then(res => {
                this.setState({
                    status: true,
                    healthArray: res.data.articles.slice(0, Math.floor(res.data.articles.length / 2))
                })
            })
    }
    ...
    render() {
        const { isLoading, lastTen, healthArray, status } = this.state
        return (
            <div>
                {status ?
                    <div>
