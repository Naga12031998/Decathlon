...
class Landingpage extends React.Component {
        ...
        window.onscroll = () => {
            ...
            if (offset === height) {
                this.timer = setTimeout(() => {
                    Axios.get(`https://newsapi.org/v2/top-headlines?country=in&apiKey=c8ca65b96c864b2aa46262c0e029520e`)
                        .then((res) => {
                            this.setState({
    ...
    componentDidMount = () => {
        Axios.get(`https://newsapi.org/v2/top-headlines?country=in&apiKey=c8ca65b96c864b2aa46262c0e029520e`)
            .then(res => {
                this.setState({
                    status: true,
                    topHeadlineIndia: res.data.articles.slice(0, Math.floor(res.data.articles.length / 2))
                })
            })
    }
    ...
    render() {
        const { isLoading, lastTen, topHeadlineIndia, status } = this.state
        return (
            <div>
                {status ?
                    <div id='news_list_container'>
                        {topHeadlineIndia.map(e => {
                            return (
                                <div className='card my-3' key={e.url}>
                                    <div className='row'>
                                        <div className='col-lg-8 col-md-12 col-sm-6 mx-3 my-3' id="card">
